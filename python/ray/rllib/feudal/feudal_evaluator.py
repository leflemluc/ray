...
from ray.rllib.utils.process_rollout import compute_advantages_feudal
from ray.rllib.feudal.loss import ProximalPolicyGraph
...
class FeudalEvaluator(PolicyEvaluator):
    ...
    def __init__(self, registry, env_creator, config, logdir, is_remote):
        ...
        self.env = ModelCatalog.get_preprocessor_as_wrapper(
            registry, env_creator(config["env_config"]), config["model_manager_filter"])
        if is_remote:
        ...
        self.manager_prev_vf_preds = tf.placeholder(tf.float32, shape=(None,))
        self.worker_prev_vf_preds= tf.placeholder(tf.float32, shape=(None,))

        self.g_sum = tf.placeholder(tf.float32, shape=(None, config["g_dim"]))
        self.carried_z = tf.placeholder(tf.float32, shape=(None, config["model_manager_filter"]["fcnet_hiddens"][-1]))
        self.s_diff = tf.placeholder(tf.float32, shape=(None, config["g_dim"]))
        ...
        def build_loss(obs, vtargets_manager, vtargets_worker, advs_manager, advs_worker,
                       acts, plog, manager_pvf_preds, worker_pvf_preds,g_sum, carried_z, s_diff):

            return ProximalPolicyGraph(self.env.observation_space, self.env.action_space, self.global_step,
                                       obs, vtargets_manager, vtargets_worker,
                                       advs_manager, advs_worker, acts,
                                       plog, manager_pvf_preds, worker_pvf_preds,
                                       g_sum, carried_z, s_diff,
                                       self.logit_dim, self.kl_coeff, self.distribution_class, self.config,
        ...
        self.par_opt = LocalSyncParallelOptimizer(
            ...
            [self.observations, self.value_targets_manager, self.value_targets_worker,
             ...
             self.actions, self.prev_logits, self.manager_prev_vf_preds, self.worker_prev_vf_preds,
             self.g_sum, self.carried_z, self.s_diff],
        ...
        with tf.name_scope("test_outputs"):
            ...
            self.total_mean_loss = tf.reduce_mean(
                tf.stack(values=[
                    policy.loss for policy in policies]), 0)
            self.manager_policy_loss = tf.reduce_mean(
                tf.stack(values=[
                    policy.manager_loss for policy in policies]), 0)
            self.manager_vf_loss = tf.reduce_mean(
                tf.stack(values=[
                    policy.manager_vf_loss for policy in policies]), 0)
            self.worker_policy_loss = tf.reduce_mean(
                tf.stack(values=[
                    policy.worker_loss for policy in policies]), 0)
            self.worker_vf_loss = tf.reduce_mean(
                tf.stack(values=[
                    policy.worker_vf_loss for policy in policies]), 0)
        ...
        self.rew_filter = MeanStdFilter((), clip=config["reward_clipping"])
        self.filters = {"obs_filter": self.obs_filter,
                        "rew_filter": self.rew_filter}
        self.sampler = SyncSampler(
            self.env, self.common_policy, self.obs_filter,
            self.config["horizon"], self.config["horizon"], feudal=True, c=self.config["c"])
    ...
    def load_data(self, trajectories, full_trace):
        return self.par_opt.load_data(
            self.sess,
            [trajectories["obs"],
             trajectories["manager_value_targets"],
             trajectories["worker_value_targets"],
             ...
             trajectories["g_sum"],
             trajectories["carried_z"],
             trajectories["s_diff"]],
            full_trace=full_trace)

    def run_sgd_minibatch(
            self, batch_index, kl_coeff, global_step, full_trace, file_writer):
        return self.par_opt.optimize(
            self.sess,
            batch_index,
            extra_ops=[
                self.total_mean_loss, self.manager_policy_loss, self.manager_vf_loss,
                self.worker_policy_loss, self.worker_vf_loss,
                self.mean_kl, self.mean_entropy],
            extra_feed_dict={self.kl_coeff: kl_coeff, self.global_step : global_step},
    ...
    def sample(self):
        ...
        while num_steps_so_far < self.config["min_steps_per_task"]:
            ...
            samples = compute_advantages_feudal(
                rollout, last_r, self.config["gamma"], self.config["gamma_internal"],
                self.config["lambda"], tradeoff_coeff=self.config["tradeoff_coeff"], c=self.config["c"])
